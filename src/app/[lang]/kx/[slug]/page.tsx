...
export default async function KxResource({ params }: Props) {
  ...
  const breadcrumbs = [
    ...
    { name: dict.global.common.kxHub || 'Knowledge Exchange', item: `/${lang}/kx/` },
            ...
            <div className="mt-12 pt-8 border-t border-white/5">
              <div className="text-center">
                <h3 className="text-xl font-bold text-white mb-3">
                  {dict.global.common.readyToScale || 'Ready to scale?'}
                </h3>
                <p className="text-white/60 mb-8 text-sm">
                  {dict.global.common.buildFoundation || 'Let’s build a foundation for growth.'}
                </p>
          ...
          {postData.previousPost && (
            <Link href={`/${lang}/kx/${postData.previousPost.id}/`} className="group">
              <Card className="bg-card/20 border-white/5 p-4 group-hover:bg-card/40 transition-colors">
                <p className="text-[10px] font-bold text-primary uppercase mb-1">{dict.pages.knowledgeHub.hub.previous || "Previous"}</p>
          ...
          {postData.nextPost && (
            <Link href={`/${lang}/kx/${postData.nextPost.id}/`} className="group">
              <Card className="bg-card/20 border-white/5 p-4 text-right group-hover:bg-card/40 transition-colors">
                <p className="text-[10px] font-bold text-primary uppercase mb-1">{dict.pages.knowledgeHub.hub.next || "Next"}</p>
