...
export default function Footer({ lang = "en", dict }: { lang?: string; dict?: Dictionary }) {
  ...
  return (
    <footer className="w-full border-t border-white/10 bg-background backdrop-blur-xl">
      <div className="container mx-auto flex flex-col items-center gap-10 py-12 max-w-6xl px-4 md:px-8">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-x-8 gap-y-6 text-sm text-muted-foreground font-medium">
          <Link 
            href={`/${lang}/privacy-policy/`} 
            className="hover:text-primary transition-colors py-3 px-2 min-h-[44px] flex items-center"
          >
            {privacyText}
          </Link>
          ...
          <div className="h-4 w-px bg-white/10 hidden sm:block"></div>
          <Link 
            href={`mailto:${companyProfile.public_company_profile.contact_details.email}`} 
            className="hover:text-primary transition-colors py-3 px-2 min-h-[44px] flex items-center"
          >
            {companyProfile.public_company_profile.contact_details.email}
          </Link>
          <div className="h-4 w-px bg-white/10 hidden sm:block"></div>
          <span className="py-3 px-2 min-h-[44px] flex items-center select-none opacity-60">
            {companyProfile.public_company_profile.primary_identifiers.vat_id_number}
          </span>
        </div>
        
        <div className="text-xs sm:text-sm text-muted-foreground/60 text-center tracking-wide">
