...
- [ ] **Consolidate the tag/container IDs** — several Google tag / container IDs have accumulated across setups; audit and keep only what feeds property 529319038. Known so far:
  - `G-ESH71F3XK7` — the **Google tag actually loaded on the site** (via gtag.js) → routes to destination `G-VL0FR2B3DH` (property 529319038). **This is the live, working path.** Verified in production (`window.google_tag_manager` shows `G-ESH71F3XK7` loaded).
    - ⚠️ **LOAD-BEARING — do NOT trash this tag or its `G-VL0FR2B3DH` destination stream.** Hardcoded in `.env` (`NEXT_PUBLIC_GA_MEASUREMENT_ID`, line 10). Trashing it makes `gtag/js?id=G-ESH71F3XK7` return **404** → library can't load → **GA down site-wide** (this happened 2026-08-14 during tag consolidation; fixed by restoring the tag from trash). To *intentionally* migrate (e.g. to `GT-5797Z722`): update the `.env` var + redeploy, don't just delete the old tag.
    - Note: `G-VL0FR2B3DH` returning **404** on `gtag/js` is **normal** — it's a destination-only measurement ID (you send `collect` with its `tid`, never load a script for it). Not a problem.
